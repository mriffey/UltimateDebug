...
!-----------------------------------------
	...
	RETURN
!-----------------------------------------
UltimateDebug.Debug PROCEDURE(STRING pDebugString)
!-----------------------------------------
...
lc_CDebugString         CSTRING(1000)
lc_COutDebugString      CSTRING(1000)
lc_Temp                 CSTRING(1000)
ll_Count                LONG
ll_End                  LONG
lb_Flag                 BYTE(FALSE)
...
ll_StringPosition       LONG
  ...
  CODE      
    ...
    ll_Count = 1  
    IF SELF.DebugNoCR   
      lc_CDebugString = CLIP(SELF.DebugPrefix) & CLIP(pDebugString) 
      IF ~SELF.DebugOff 
        OutputDebugString(lc_CDebugString)
      END  
      IF SELF.SaveToFile
    ...
    ELSE
      ...
      ELSE
        ...
        Loop Until lb_Flag
          If Len(Clip(pDebugString)) - ll_Count < 999 Then
          ...
          LOOP
            ...
            IF ll_StringPosition                                      
              lc_COutDebugString = CLIP(SELF.DebugPrefix) & SUB(lc_CDebugString,1,ll_StringPosition - 1)
              ...
              CYCLE
            END
            lc_CDebugString = CLIP(SELF.DebugPrefix) & lc_CDebugString
            IF SELF.SaveToFile
              ASC1:String = '[' & FORMAT(TODAY(),@D17) & '-' & FORMAT(CLOCK(),@T7) & ']- ' & lc_CDebugString
              ADD(ASC1:ASCIIFile)
            END
...
!  cs_DebugString = FORMAT(SELF.GetEventDescr(),@s30) & EVENT() 
    ...
    SELF.DebugNoCR = TRUE
    SELF.debug(clip(cs_DebugString)) 
